...
func _draw():
	# Draw charge pulse
	if (pull_time > CHARGE_DELAY and pull_time < CHARGE_DELAY + 10):
	...
	if path.size() > 2:
		draw_polyline(path.slice(0, 3), Color.hex(0xea1f1f55))
	if path.size() == 5:
		draw_polyline(path.slice(2, 5), Color.hex(0xfb1818aa))
